...
def buscar_alternativas(desc_buscar, umbral, catalogos, stocks):
    ...
    with st.spinner(f"Buscando con similitud ≥ {umbral}%..."):
        ...
        # ========== 3. BUSCAR PRECIO (por SKU o por DESCRIPCIÓN con CONTROL DE COLOR) ==========
        for sku, data in resultados.items():
            if data['precio'] == 0:
                encontrado = False
                
                # PRIMERO: Búsqueda exacta por SKU
                for cat in catalogos:
                    df = cat['df']
                ...
                # SEGUNDO: Buscar por descripción similar respetando color
                if not encontrado and data['descripcion']:
                    ...
                    for cat in catalogos:
                        ...
                        for _, row in df.iterrows():
                            ...
                            # Verificar color coincide
                            if color_producto:
                                if color_producto.lower() not in desc_cat:
                                    continue
                            
                            # Calcular similitud
                            similitud = calcular_similitud(data['descripcion'].lower(), desc_cat)
        ...
        for j, col in enumerate(cols):
            ...
            if idx < len(resultados_lista):
                ...
                with col:
                    ...
                    # Mostrar precio con información de origen
                    if r['precio'] > 0:
                        precio_texto = f"💰 S/ {r['precio']:.2f}"
                        if r.get('sku_match'):
                            precio_texto += f" <span style='color:#999; font-size:9px;'>(via {r['sku_match']})</span>"
                        st.markdown(f'<p style="color:#e67e22; font-size:13px; font-weight:bold; margin:6px 0 4px 0;">{precio_texto}</p>', unsafe_allow_html=True)
                    ...
                    # Selector de cantidad con formulario
...
def buscar_sku_exacto(sku_buscar, catalogos, stocks):
    ...
    if st.button(f"📋 Enviar SKU {sku_limpio} al MODO MASIVO", key="exact_sku_send", use_container_width=True):
        ...
        st.success(f"✅ SKU {sku_limpio} enviado al MODO MASIVO")# ui/tabs/skuscraper_tab.py
import streamlit as st
import pandas as pd
from difflib import SequenceMatcher

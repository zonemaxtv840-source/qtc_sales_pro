    ...
    with col_b1:
        if st.button("🚀 Procesar lista", type="primary", use_container_width=True):
            if not texto_bulk:
                ...
                procesar_lista_xiaomi(texto_bulk) if st.session_state.modo == "XIAOMI" else procesar_lista_ugreen(texto_bulk)
...
def procesar_lista_xiaomi(texto_bulk):
    pedidos = []
    ...
    with st.spinner("Procesando..."):
        ...
        <div class="counter-summary">
            ...
            <div class="counter-item" style="color:#f44336;">❌ Sin precio: <strong>{total_ingresados - encontrados}</strong></div>
            <div class="counter-item" style="color:#f44336;">🚫 Sin stock: <strong>{total_ingresados - con_stock}</strong></div>
        </div>
        """, unsafe_allow_html=True)
        
        st.success(f"✅ Procesados {len(pedidos)} productos")

def procesar_lista_ugreen(texto_bulk):
    pedidos = []
    for line in texto_bulk.strip().split('\n'):
        line = line.strip()
        if ':' in line:
...
def mostrar_resultados_bulk():
    if 'resultados_bulk' not in st.session_state or not st.session_state.resultados_bulk:
        return
